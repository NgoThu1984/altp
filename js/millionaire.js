...
var MillionaireModel = function(data) {
    ...
    // The current money obtained
 	...
 	self.fifty = function(item, event) {
 		if(self.transitioning || self.usedFifty())
 			return;
 		self.usedFifty(true);
 		$(event.target).addClass('used').fadeOut('slow');
 	...
 	self.rightAnswer = function(elm) {
 		...
 		setTimeout(function() {
 			var amount = $(".levels-sidebar li.active").data('amt');
 			self.money(amount);
 			
 			if(self.level() + 1 > 15) {
 				$("#game").fadeOut('slow', function() {
 					$("#game").removeClass('active');
 					$("#result-text").text('CHÚC MỪNG CHIẾN THẮNG!');
 					$("#final-money").text('Bạn đã giành được: $' + self.formatMoney());
 					$("#game-over").fadeIn('slow').addClass('active');
 				});
 			} else {
 				self.level(self.level() + 1);
 				$(".answer-card").removeClass('correct wrong');
 				$(".answer-card").show();
 	...
 	self.wrongAnswer = function(elm) {
 		...
 		setTimeout(function() {
 			$("#game").fadeOut('slow', function() {
 				$("#game").removeClass('active');
 				$("#result-text").text('RẤT TIẾC, GAME OVER!');
 				$("#final-money").text('Số tiền thưởng cuối cùng: $' + self.formatMoney());
 				$("#game-over").fadeIn('slow').addClass('active');
 				self.transitioning = false;
 			});
 		}, 1000);
...
$(document).ready(function() {
	...
	$.getJSON("questions.json", function(data) {
		for(var i = 1; i <= data.games.length; i++) {
			var title = data.games[i-1].title || i;
			$("#problem-set").append('<option value="' + i + '">' + title + '</option>');
		}
		$("#pre-start").addClass('active').show();
		$("#start").click(function() {
			...
			$("#pre-start").fadeOut('slow', function() {
				$("#pre-start").removeClass('active');
				startSound('background', true);
				$("#game").fadeIn('slow').addClass('active');
			});
		});
	});
});